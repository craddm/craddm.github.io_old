...
```{r basicPlot, fig.height = 3, fig.width = 6}
...
```{r CIPlot,fig.height = 3, fig.width = 6}
levCat.plot+
...
```{r indivNoGroup, fig.height = 4, fig.width = 6}
levCat.plot+
  geom_line(aes(group = interaction(Subject,variable),colour = variable,alpha = 0.2))+
  guides(alpha= "none")+
  theme_minimal()+
...
```{r condSplit, fig.height = 4, fig.width = 8}
levCat.plot+
  facet_wrap(~variable)+
...
```{r condSplitNoMean, fig.height = 4, fig.width = 8}
levCat.plot+
  facet_wrap(~variable)+
  geom_line(aes(group = Subject),alpha = 0.3)+
  theme_minimal()+
...
```{r diffPlot, fig.height = 3, fig.width = 6}
levCat.plot+
...
```{r indivDiffPlot, fig.height = 3, fig.width = 6}
...
### References
<div id="craddock">
<p>Craddock, M., Martinovic, J., & Müller, M. M. (2015). Early and late effects of objecthood and spatial frequency on event-related potentials and gamma band activity. BMC Neuroscience, 16(1), 6. <a href ="http://doi.org/10.1186/s12868-015-0144-8">doi</a></p>
</div>