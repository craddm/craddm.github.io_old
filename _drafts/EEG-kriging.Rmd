...
lzn.fit <- fit.variogram(lzn.vgm,model = vgm(1,"Exp"))
...
lzn.kriged %>% as.data.frame %>%
  ggplot(aes(x=x, y=y)) + geom_raster(aes(fill=var1.var)) + coord_equal() +
  stat_contour(aes(z = var1.var),binwidth = 0.3)+
  scale_fill_gradientn(colours = jet.colors(10),guide = "colourbar",oob = squish)+
