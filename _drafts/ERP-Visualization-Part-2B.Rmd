---
title: "ERP visualization with three conditions: part 2B"
...
In part 2A I took a look at visualizing ERPs from two conditions at a single electrode. This time I'm going to look at three conditions. As in the previous post, I'll assume a basic familiarity with ERPs.
...
ERP.plot +
  stat_summary(fun.y = mean,geom = "line",size = 1,aes(colour = Frequency,linetype = Object)) +
  labs(x = "Time (ms)",y = expression(paste("Amplitude (",mu,"V)")),colour = "") +
  geom_vline(xintercept = 0,linetype = "dashed" ) +
  geom_hline(yintercept = 0,linetype = "dashed")
```
...
And since there's no way to tell which line belongs to which participant, it's impossible to know if it's the same participants showing the same patterns across conditions (although I'd say it's very likely in the early parts of the ERP at least). And of course, what I'm really interested in here are the within-subject differences across conditions.
...
ERPdiff.plot+
  ...
  labs(x = "Time (ms)", y = expression(paste("Amplitude (",mu,"V)")),colour = "")+
  stat_summary(fun.y = mean,geom = "line",size = 1,aes(y = GAdiff,colour = Frequency))+
  geom_line(aes(x = Time, y = crit-2),na.rm = TRUE,size = 2)+
  geom_vline(xintercept = 0,linetype = "dashed" )+
  geom_hline(yintercept = 0,linetype = "dashed")


#    stat_summary(fun.data = mean_cl_boot,geom = "ribbon",size = 1,aes(y = GAdiff,fill = Frequency),alpha = 0.4)+
  #stat_summary(fun.y = mean,geom = "line",size = 1,aes(y = GAdiff,fill = Frequency),alpha = 0.4)+  #stat_summary(fun.y = mean,geom= "line",aes(x = Time,y = crit-3),na.rm = TRUE,size = 2)+


```

This shows that, in the uncorrected p values at least, there is a significant effect of frequency around 60 ms, from around 140 to 250 ms, and from around 290 - 330 ms. Of course, we can't tell from an F-test alone which means are significantly different from each other at any given time point, even if we can hazard a reasonable guess. It's pretty clear that LSF are different from the HSF and BB images in the 140 to 250 ms window, for example.
