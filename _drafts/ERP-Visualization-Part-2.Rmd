...
library(ggplot2)
library(reshape2)
levCatGAall <- read.csv("C:/Users/pscmpc/Dropbox/BlogStuff/RMarkdown/levCatGAall.csv",header=FALSE)
...
Now it's becoming a real mess. It's really difficult to tell the conditions apart properly in the individual traces. Poor choice of colours? Sure, but I tried rendering this plot under multiple colour schemes and it simply didn't help. And this problem can only get worse when you add more conditions. Let's split the conditions up.
...
Typically we'd be running an F-test to check for differences among the means. The first plot we can do corresponds to 

What we need for that is the difference between each condition mean and the *grand* mean, the mean across all conditions. 

In our case, we now have BB-HSF, BB-LSF, and HSF-LSF.
...
Ftests <- by(levCatGAall,levCatGAall$Time,function (x) summary(aov(amplitude~Object*Frequency + Error(Subject/(Object*Frequency)),data = x))) #runs separate repeated measures ANOVA on each timepoint

unlist(summary(test.aov)[[3]])[[7]] #F-ratio
unlist(summary(test.aov)[[3]])[[9]] #p-value

FtestPurr <- levCatGAall %>%
  split(.$Time) %>%
  #map(~t.test(amplitude~Object,paired = TRUE,data = .)) %>%
  map(~t.test(amplitude~Object,paired = TRUE,data = .)) %>%
  map_dbl("p.value")

FtestPurrAOV <- levCatGAall %>%
  split(.$Time) %>%
  map(~ aov(amplitude~Frequency+Error(Subject/Frequency),data = .)) %>%
  map(summary) %>%
  map_int("Pr(>F)")
...
ERPdiff.plot+
  ...
  labs(x = "Time (ms)", y = expression(paste("Amplitude (",mu,"V)")),colour = "")+
  #stat_summary(fun.data = mean_cl_boot,geom = "ribbon",size = 1,aes(y = GAdiff,fill = Frequency),alpha = 0.4)+
  stat_summary(fun.y = mean,geom = "line",size = 1,aes(y = GAdiff,colour = Frequency,group = interaction(Subject,Frequency)))+
  geom_vline(xintercept = 0,linetype = "dashed" )+
  geom_hline(yintercept = 0,linetype = "dashed")
