...
Now we have three subplots, each informative about a particular condition. The mental gymnastics to work out where the differences are becoming harder, and I find myself relying on the condition means. 

## Difference waves

In the two condition post, life was simple. Two conditions only need a single difference wave. But as the number of conditions increases, the number of pairwise differences increases.

Typically we'd be running an F-test to test whether any of the means differ from each other, and, if they do, we'd then run post-hoc t-tests to check which pairs of means differ. A simple starting point is thus the difference between each condition mean and the *grand* mean, the mean across all conditions. 
