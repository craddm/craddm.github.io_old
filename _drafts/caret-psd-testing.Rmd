...
clean_data <- read_csv('F:\\Dropbox\\BlogStuff\\EEG_data\\epochs_GAT_clean.csv', progress = FALSE) %>%
  gather(electrode,amplitude, -condition, -epoch, -time) %>%
  spread(electrode,amplitude) %>%
  separate(condition, c("light","touch","report"), sep = "/")

epoch_list <- select(clean_data, epoch, report)

all_psd_three %>% spread(electrode, spec) %>% left_join(epoch_list, by = "epoch")
...
z <- all_psd_three %>%
  spread(electrode, spec) %>%
  left_join(epoch_list, by = "epoch") %>%
  select(-epoch) %>%
  nest(-freqs) %>%
  mutate(fit = map(data, ~train(report ~ ., data = .x, preProcess = "scale",
                                method = "glmnet", family = "binomial", trControl = fitControl)))

ZZ <- data.frame(Accuracy = map(z$fit,"resample") %>%
                   map("Accuracy") %>%
                   map_dbl(~mean(.x)), time = z$freqs)
ZZ$freqs <- z$freqs

ggplot(ZZ, aes(freqs, Accuracy))+geom_line()
```
