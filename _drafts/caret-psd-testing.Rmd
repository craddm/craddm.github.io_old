...
epoch_list <- select(clean_data, epoch, light)
table(epoch_list$light)
...
z <- all_psd_three %>%
  ...
  select(-epoch) %>%
  nest(-freqs) %>%
  mutate(fit = map(data, ~train(light ~ ., data = .x, preProcess = c("center","scale"),
                                method = "glmnet", family = "binomial", trControl = fitControl)))

ZZ <- data.frame(Accuracy = map(z$fit,"resample") %>%
                   map("Accuracy") %>%
                   map_dbl(~mean(.x)), freqs = z$freqs)
ZZ$freqs <- z$freqs

ZZ %>% filter(freqs > 4) %>%
  ggplot(aes(freqs, Accuracy))+geom_line()+ylim(0,1)
```
