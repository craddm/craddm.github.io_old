...
electrodeLocs$radianTheta <- radianTheta
electrodeLocs <- electrodeLocs %>%
  mutate(x = .$radius*sin(radianTheta),
         y = .$radius*cos(radianTheta))
ggplot(electrodeLocs,aes(radianTheta,radius,label = electrode))+geom_text()+theme_bw()
...
```{r interpTopo}
tmpTopo <- with(timeZero, interp(x = x, y = y, z = amplitude,xo = seq(min(x)*2, max(x)*2,length = 164), yo = seq(min(y)*2, max(y)*2, length = 100),linear = FALSE,extrap = TRUE)) 
...
datmat2 <- data.frame(expand.grid(x = seq(min(timeZero$x)*2,max(timeZero$x)*2, length = 100), y = seq(min(timeZero$y)*2,max(timeZero$y)*2, length = 100)))
...
rmax <- .75
gridRes <- 100
testDat<- data.frame(x = timeZero$x,
                     y = timeZero$y,
                     z = timeZero$amplitude)
...
for (i in 1:gridRes){
  for (j in 1:gridRes) {
    test4 <- abs((xo2[i,j] + sqrt(as.complex(-1))*yo2[i,j]) - xy)
    g <- (test4^2) * (log(test4)-1)
    outmat[i,j] <- g %*% weights
#    test4[i,j]<- test4[i,j] - xy
  }
}
finalOut <- list(x = xo2[,1],y = yo2[1,],z = outmat)
#}
```
...
datmat5 <- interpV4[interpV4$incircle,]
datmat5 %>%
  ggplot(aes(x = x,y= y,fill = amplitude))+
  geom_raster(interpolate = TRUE)+
  stat_contour(aes(z = amplitude),binwidth = 0.15)+
  theme_topo()+
  scale_fill_gradientn(colours = jet.colors(10),limits = c(-.73,.73),guide = "colourbar",oob = squish)+
  scale_colour_gradientn(colours = jet.colors(10),limits = c(-.73,.73),guide = "colourbar",oob = squish)+
  geom_point(data =timeZero,aes(x,y,colour = amplitude),size = 3)+
...
coordinates(hmmm) <- ~x + y
lzn.kriged <- krige(z~x+y,spatialdat,hmmm,model=lzn.fit)
lzn.kriged %>% as.data.frame %>%
  ggplot(aes(x=x, y=y)) + geom_tile(aes(fill=var1.pred)) + coord_equal() +
  stat_contour(aes(z = var1.pred),binwidth = 0.15)+
  scale_fill_gradientn(colours = jet.colors(10),limits = c(-.73,.73),guide = "colourbar",oob = squish)+
  scale_colour_gradientn(colours = jet.colors(10),limits = c(-.73,.73),guide = "colourbar",oob = squish)+
  geom_point(data =timeZero,aes(x,y,colour = amplitude),size = 3)+
