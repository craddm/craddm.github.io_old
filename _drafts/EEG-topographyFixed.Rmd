...
datmat2 <- data.frame(expand.grid(x = seq(round(min(timeZero$x)),round(max(timeZero$x)), length = 100), y = seq(min(timeZero$y),max(timeZero$y), length = 100)))
...
v4interp <- function(x,y,z) {
  #[x, y, v] = mergepoints2D(x,y,v);

xy <- x + y*sqrt(as.complex(-1))
