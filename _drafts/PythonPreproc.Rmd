...
raw = mne.io.read_raw_edf('D:\Dropbox\EEGData\Leeds\InteroExtero\S115\Extero\S115ExteroSSDTB2.bdf',preload = True,eog = eogChans,misc = miscChans)
...
raw_LM, _ = mne.io.set_eeg_reference(raw, ['EXG5', 'EXG6'])
...
baseline = (None, 0)
evoked_no_ref = mne.Epochs(raw_no_ref, events=events, event_id=event_id, tmin=tmin,
                    tmax=tmax, picks=picks, add_eeg_ref=False,preload = True)
evoked_no_ref = evoked_no_ref.average()
#epochs_avg = epochs.average()

evoked_avg_ref = mne.Epochs(raw_car, events=events, event_id=event_id, tmin=tmin,
                    tmax=tmax, picks=picks, add_eeg_ref=False,preload = True)
evoked_avg_ref = evoked_avg_ref.average()
evoked_LM_ref = mne.Epochs(raw_LM, events=events, event_id=event_id, tmin=tmin,
                    tmax=tmax, picks=picks, add_eeg_ref=False,preload = True)
                    
evoked_LM_ref = evoked_LM_ref.average()
#evoked_avg_ref = epochs_avg_ref.average()            

#epochs_avg.plot_topomap(0.171, show_names=True, colorbar=False,
 #                   size=6, res=67,vmin = -5, vmax = 5, outlines = 'skirt')

df = evoked_no_ref.to_data_frame()
df.to_csv('C:\\Users\\Matt\\Documents\\Github\\ExploringERPs\\evokedNoRef.csv')
#dfRaw = epochs.to_data_frame()
#dfRaw.to_csv('C:\\Users\\Matt\\Documents\\Github\\ExploringERPs\\my_epochs.csv')
dfAvgRef = evoked_avg_ref.to_data_frame()
dfAvgRef.to_csv('C:\\Users\\Matt\\Documents\\Github\\ExploringERPs\\evokedAvgRef.csv')

dfLMRef = evoked_LM_ref.to_data_frame()
dfLMRef.to_csv('C:\\Users\\Matt\\Documents\\Github\\ExploringERPs\\evokedLMRef.csv')

```

```{r}
newdat <- read_csv('C:\\Users\\Matt\\Documents\\Github\\ExploringERPs\\evokedNoRef.csv')
newdat2 <- read_csv('C:\\Users\\Matt\\Documents\\Github\\ExploringERPs\\evokedLMref.csv')
newdat3 <- read_csv('C:\\Users\\Matt\\Documents\\Github\\ExploringERPs\\evokedAvgRef.csv')
EEGdat <- gather(newdat,electrode,amplitude,-time)
EEGdat2 <- gather(newdat2,electrode,amplitude,-time)
EEGdat3 <- gather(newdat3,electrode,amplitude,-time)
...
levCat2.plot <- ggplot(EEGdat2,aes(time,amplitude))+
  geom_line(aes(colour = electrode))
levCat3.plot <- ggplot(EEGdat3,aes(time,amplitude))+
  geom_line(aes(colour = electrode))
...
allData <- EEGdat3 %>% left_join(electrodeLocs, by = "electrode")
allData
```
...
gg <- welchPSD(yts,seglength = .6,method = "median")
